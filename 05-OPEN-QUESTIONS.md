...
This is the central implementation question. In the theory, Ω is defined abstractly: "the act of a conscious agent attending to an element generates new elements." But what does this mean computationally?
...
1. **Formalise the observation operator (Q1)** — Observation-as-perturbation works for numerical optimisation. The next step is to generalise this to non-numerical domains: can Ω be implemented as type inference in a dependently-typed system?

2. **Implement coordinate-wise devolution** — High-dimensional spaces motivate this. Split parameters into sub-groups of 1–3, run independent Genesis ticks on each, reunify. This is the first test of the devolution mechanism (Q5).

3. **Explore correlated perturbation** — Use covariance estimation from elite agents (analogous to CMA-ES) to generate perturbations aligned with promising search directions. The covariance matrix IS the relational structure R of the platonic space.

4. **Test Conjecture C1 (stability landscape)** — Define "platonic stability" formally and compare to nuclear binding energy. This is the most directly testable conjecture.

5. **Design the hybrid architecture (Note 6)** — How would Genesis Learning's platonic space interface with a neural network? What data format would the bridge use?

---

## Theoretically-Derived Questions

*These questions were discovered by analysing the algorithm against its axioms. They represent gaps where the theory doesn't prescribe an answer and pragmatic choices must be made.*

### Q6: The Normalisation–Arithmetic Paradox

**Problem:** Function learning requires `embed(a) + embed(b) ≈ embed(a+b)` (additive arithmetic). But if all embeddings are normalised to the unit sphere for consistent nearest-neighbour retrieval, then `|embed(x)| = 1` for all x, so `|embed(a) + embed(b)| ≈ √2`, which after re-normalisation becomes a direction, not a value.
...
**Proposed resolution:** Dual-face architecture (identity + arithmetic embeddings per element) or logarithmic value encoding (where multiplication becomes addition).

**Status:** Critical — this is a fundamental tension in the theory.

### Q7: Should Element Creation Enforce Conservation (G4)?

**Problem:** When elements are introduced into the space from outside (e.g., seeded vocabulary, user-provided concepts), they may be created WITHOUT complements, violating G4. The tick pipeline creates complements, but externally-injected elements bypass it.

**The question:** Is external injection a form of observation (subject to G4), or is it a meta-operation (exempt)?
...
**Proposed experiment:** Compare training accuracy with and without conservation on element creation. If complements improve arithmetic, the theory is validated.

**Status:** Unanswered. Affects total charge drift and complement space utilisation.

### Q8: Concatenation vs. Averaging for Multi-Argument Composition

**Problem:** The embedding space has complementary anti-dimensions (ontologically justified by the +1/−1 complement structure). Currently, multi-argument composites use AVERAGING (lossy). The natural alternative is CONCATENATION: one argument on the arithmetic face, the other on the semantic face.

**The question:** Does the complement structure prescribe a composition operator, or is the choice arbitrary?

**Hypothesis:** Concatenation is axiom-aligned. The complement space was designed for two-face representation: positive (+1) and negative (−1). A binary function naturally occupies both faces — one argument on the positive face, the other on the negative face.

**Prediction:** Concatenation will produce more distinguishable composites than averaging.

**Status:** Untested. This is the most promising path to multi-argument accuracy.

### Q9: Does Embedding Update Conflict with Function Learning?

**Problem:** The embedding update process pulls embeddings toward graph neighbours (message-passing). During function learning, a separate process pulls output embeddings toward `input + T` (the transform vector). These forces may conflict.

**The question:** Should graph-based embedding updates be disabled during function learning?

**Evidence:** After integration ticks, numeric embeddings can drift from their encoded positions toward the mean of their graph neighbourhood, degrading the structured numeric encoding.
